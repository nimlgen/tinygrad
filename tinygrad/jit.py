...
from tinygrad.helpers import DEBUG, NOSTAT, DType, merge_dicts, getenv
...
class TinyJit:
  def __init__(self, fxn:Callable):
    ...
    self.batch_executor: BatchExecutor = None
  ...
  def __call__(self, *args, **kwargs) -> Any:
    ...
    if self.cnt >= 2:
      ...
      if self.batch_executor:
        for j,(prg, pargs, variables) in enumerate(self.jit_cache):
        ...
        if not NOSTAT:
          for j,(prg, pargs, variables) in enumerate(self.jit_cache): prg.update_stat_after_call(variables)
      else:
