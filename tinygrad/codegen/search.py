...
def kernel_optimize_search(k:Linearizer, create_k:Callable[[], Linearizer], to_prg, baseline):
  ...
  opts = []
  if k.first_reduce < k.shape_len: # HACK: Grouped reduces do not work with other locals. More chances to mutate to 1, so locals can be used.
    g_opts = ng.p.TransitionChoice([(0,s,"G") for s in get_divisors(k.full_shape[k.first_reduce], min_div=16, max_div=256) if all(st.shape[k.first_reduce] % s == 0 or st.shape[k.first_reduce] == 1 for st in k.sts)])
    opts.append(ng.p.TransitionChoice([g_opts]+[(0,1,"G")]*4, transitions=(0.3, 0.7))) # 20% of chance getting meaningful value in group_for_reduce. 70% to leave state, 30% to stay.
  for i in range(k.first_reduce):
    ...
    opts.append(ng.p.TransitionChoice([(i,s,"L") for s in get_divisors(k.full_shape[i], max_div=256)]))
