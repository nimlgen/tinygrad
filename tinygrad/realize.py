...
def lower_schedule_item(si:ScheduleItem) -> Optional[JITRunner]:
  ...
  if si.ast.op in {LoadOps.SYNC, LoadOps.COPY} and si.out.device.startswith("HSA") and si.inputs[0].device.startswith("HSA"):
    from tinygrad.runtime.ops_hsa import HSABufferXfer
    # Our HSA runtime handles synchronization
    if si.ast.op is LoadOps.SYNC: return None
    if si.ast.op is LoadOps.COPY and type(Device[si.out.device]) is type(Device[si.inputs[0].device]): return HSABufferXfer(si)
  if si.ast.op is LoadOps.COPY:
    ...
    if si.inputs[0].device.startswith("DISK"): return None #BufferRead()
    return BufferCopy()
