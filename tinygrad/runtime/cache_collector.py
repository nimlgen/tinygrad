...
class _CacheCollector:
  class BufferPlaceholder:
    ...
    def get_rawbuf(self):
      # NOTE: If ref is valid, it means the actual buffer is still in use (not even reused from the cache). To keep it valid, we use this buffer directly.
      return self.ref() if self.alive() else self.alloc_rawbuf()

  def __init__(self):
    self.cache: Optional[List[Tuple[Callable, Any]]] = None
    self.placeholders: Dict[RawBuffer, _CacheCollector.BufferPlaceholder] = {} # Rawbuffers are replaced with placeholders to allow freeing of the real buffer while collecting cache.
    ...
    self.last_placeholder_index: Dict[_CacheCollector.BufferPlaceholder, int] = {} # Last index where the placeholder is used as output. This allows tracking when we need to stick to the original buffer if it is still alive.
  def start(self):
    self.cache = []
    self.placeholders = {}
    self.last_buftype = {}
    self.last_placeholder_index = {}
  def add(self, prg, rawbufs):
    ...
    self.placeholders.setdefault(get_ref(rawbufs[0]), _CacheCollector.BufferPlaceholder(rawbufs[0])).ref = ref(rawbufs[0]) # Updating placeholder ref to point to reallocated buffer(or wrapper).
  ...
  def finish(self):
    ...
    for j,(p,cached_bufs) in enumerate(self.cache):
      if cached_bufs[0].__class__ is _CacheCollector.BufferPlaceholder and cached_bufs[0].alive():
        ...
        li = self.last_buftype.get(self._buftype_key(cached_bufs[0]), -1)
        if li < j or self.last_placeholder_index[cached_bufs[0]] == j: placeholder_mapper[cached_bufs[0]] = cached_bufs[0].get_rawbuf()
      ...
      placeholder_mapper.update({buf: buf.get_rawbuf() for buf in cached_bufs if buf.__class__ is _CacheCollector.BufferPlaceholder and buf not in placeholder_mapper})
