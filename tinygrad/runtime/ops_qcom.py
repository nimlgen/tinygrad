...
from tinygrad.device import BufferSpec, Buffer, Device, LRUAllocator, TinyELF
...
class QCOMAllocator(LRUAllocator['QCOMDevice']):
  def _alloc(self, buf:Buffer, opaque=None):
    return self.dev._gpu_map(buf.options.external_ptr, buf.nbytes) if buf.options.external_ptr else self.dev._gpu_alloc(buf.nbytes)
  def _free(self, buf:Buffer): self.dev._gpu_free(buf)
...
class QCOMDevice(HCQ2Compiled):
  ...
  def _gpu_alloc(self, size:int, flags:int=0, uncached=False, fill_zeroes=False) -> tuple:
    ...
    return va_addr, MMIOInterface(va_addr, size, fmt='B'), (alloc, True)

  def _gpu_map(self, ptr:int, size:int) -> tuple:
    ...
    try:
      ...
      return mi.gpuaddr + (ptr - ptr_aligned), MMIOInterface(ptr, size, fmt='B'), (mi, False)
    except OSError as e:
      if e.errno == 14: return ptr, MMIOInterface(ptr, size, fmt='B'), (None, False)
      raise RuntimeError("Failed to map external pointer to GPU memory") from e

  def _gpu_free(self, b:Buffer):
    if b.data[0] is None: return # external (gpu) ptr
    if not b.data[1]: kgsl.IOCTL_KGSL_SHAREDMEM_FREE(self.fd, gpuaddr=b.data[0].gpuaddr) # external (cpu) ptr
    else:
      kgsl.IOCTL_KGSL_GPUOBJ_FREE(self.fd, id=b.data[0].id)
      FileIOInterface.munmap(b.gpu, b.data[0].mmapsize)
