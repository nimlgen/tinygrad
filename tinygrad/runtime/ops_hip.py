...
class HIPAllocator(LRUAllocator):
  ...
  def copyin(self, dest, src: memoryview):
    check(hip.hipSetDevice(self.device.device_id))
    check(hip.hipMemcpy(dest, from_mv(src), src.nbytes, hip.hipMemcpyHostToDevice, None))
  def copyout(self, dest:memoryview, src):
    self.device.synchronize()
    check(hip.hipSetDevice(self.device.device_id))
    check(hip.hipMemcpy(from_mv(dest), src, dest.nbytes, hip.hipMemcpyDeviceToHost))
  def transfer(self, dest, src, sz:int, **kwargs):
    check(hip.hipSetDevice(self.device.device_id))
...
class HIPDevice(Compiled):
  def __init__(self, device:str=""):
    ...
    super().__init__(self.device_id, HIPAllocator(self), AMDRenderer(), AMDCompiler(self.arch), functools.partial(HIPProgram, self))
