...
class PCIIfaceBase:
  ...
  def map(self, b:Buffer) -> tuple[int, Any]: # another device's memory at the same address: (gpu, the mapped range)
    if Device[b.device].allocator.host: # the covering pages of the host memory
      ...
      lo, size, snooped, uncached = b.gpu, b.data.mapping.size, True, b.data.mapping.uncached
      if b.data.mapping.aspace is AddrSpace.SYS: paddrs, aspace = b.data.mapping.paddrs, AddrSpace.SYS
      else: paddrs, aspace = ifa.p2p_paddrs(b.data.mapping.paddrs)
