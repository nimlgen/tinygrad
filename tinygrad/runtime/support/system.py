...
class PCIIfaceBase:
  ...
  def map(self, b:Buffer) -> BufferStorage:
    if b.device.split(":")[0] in {"CPU", "PYTHON", "NPY"}:
      if self.remote is not None: raise RuntimeError(f"host memory is not on the node of {self.dev.device}")
...
class RemoteCmd(enum.IntEnum):
  ...
  LOAD_PROG, EXEC_PROG = range(13, 15)
...
class RemoteMMIOInterface(MMIOInterface):
  ...
  def __getitem__(self, k):
    ...
    data = self.dev.rpc(cmd, self.off + st, en - st, bar=self.residx, readout_size=en - st)[2]
  ...
  def __setitem__(self, k, v):
    ...
    self.dev._post(self.dev.sock, cmd, self.off + st, len(data), dev=self.dev.dev_id, bar=self.residx, payload=data)
...
class RemotePCIDevice(PCIDevice):
  ...
  def alloc_sysmem(self, size:int, vaddr:int=0, contiguous:bool=False) -> tuple[MMIOInterface, list[int]]:
    n, handle, _ = self.rpc(RemoteCmd.MAP_SYSMEM, size, int(contiguous), vaddr)
    host_va, *paddrs = struct.unpack(f'<{n // 8}Q', self._recvall(self.sock, n))
    return RemoteMMIOInterface(self, handle, host_va, size, sysmem=True), paddrs
