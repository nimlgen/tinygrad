...
class IHRing():
  def __init__(self, adev, doorbell_index, is_1=False):
    ...
    self.vm = self.adev.mm.valloc(262144)
    self.rptr_vm = self.adev.mm.valloc(0x1000)
    self.wptr_vm = self.adev.mm.valloc(0x1000)
...
class IH_IP:
  ...
  def ih_v6_0_enable_ring(self, ring, is_1):
    self.adev.wreg(ring.ih_rb_base, (ring.vm.vaddr >> 8) & 0xffffffff)
    self.adev.wreg(ring.ih_rb_base_hi, (ring.vm.vaddr >> 40) & 0xff)

    cntr = 0xC0310120 if not is_1 else 0xC0100320
    self.adev.wreg(ring.ih_rb_cntl, cntr)

    if not is_1:
      self.adev.wreg(ring.ih_rb_wptr_addr_lo, ring.wptr_vm.vaddr & 0xffffffff)
      self.adev.wreg(ring.ih_rb_wptr_addr_hi, ring.wptr_vm.vaddr >> 32)
  ...
  def ih_v6_0_irq_init(self):
    ...
    self.adev.regS2A_DOORBELL_ENTRY_1_CTRL.write(0x00057801)
    ...
    self.adev.regIH_STORM_CLIENT_LIST_CNTL.write(0x40000)
    ...
    self.adev.regIH_INT_FLOOD_CNTL.write(0x8)
    self.adev.regIH_MSI_STORM_CTRL.write(0x3)
    self.adev.regIH_RING1_CLIENT_CFG_INDEX.write(0x0)
    self.adev.regIH_RING1_CLIENT_CFG_DATA.write(0x1000a)
