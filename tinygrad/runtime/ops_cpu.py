...
class CPUProgram(Program['CPUDevice']):
  ...
  def __call__(self, *bufs:Buffer, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1),
    ...
    if self.lvp:
      ...
      struct.pack_into(f'<3I{len(bufs)}Q', lvp_args, 0, *data64_le(addr+12), (len(bufs)+len(vals))*2, *[b.addr(self.dev.device) for b in bufs])
      for v,(off,dt) in zip(vals, TinyELF.iter_sig(self.signature[-len(vals):], len(bufs)*8)): struct.pack_into(f'<{dt.fmt}', lvp_args, 12+off, v)
      self.fxn(addr)
    else:
      args = [*[b.addr(self.dev.device) for b in bufs], *cast(tuple[int, ...], vals)]
...
class HostAllocator(LRUAllocator): # CPU, PYTHON and NPY: host memory, an mmap when it is ours
  host = True
