...
class ASTRunner:
  ...
  def __call__(self, rawbufs:List[RawBuffer], var_vals:Optional[Dict[Variable, int]]=None, jit=False, force_wait=False, **kwargs) -> Optional[float]:
    ...
    if et := self.clprg((global_size + [1]*(3-len(global_size))) if global_size is not None else None,
                        ...
                        *rawbufs, *var_vals.values(), wait=force_wait or DEBUG>=1, **kwargs): GlobalCounters.time_sum_s += et
