...
import functools, itertools, operator, time
...
def apply_graph_to_jit(jit_cache: List[JitItem], input_rawbuffers: List[Buffer], var_vals: Dict[Variable, int]) -> List[JitItem]:
  ...
  def flush_batch():
    ...
    try:
      ...
      # max_batch_size = min(max_batch_size * 2, getenv("JIT_MAX_BATCH_SIZE", 4096))
...
class TinyJit(Generic[ReturnType]):
  ...
  def __call__(self, *args, **kwargs) -> ReturnType:
    ...
    if self.cnt >= 2:
      ...
      # print(f"jitpass: {(time.perf_counter_ns() - st)*1e-6} ms")
    elif self.cnt == 1:
      # jit capture
