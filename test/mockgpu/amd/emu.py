...
class WaveState:
  ...
  def __init__(self, n_lanes: int, wave_size: int = 32):
    ...
    if wave_size == 64:
      ...
      ctypes.memset(self.accvgpr_buf.gpu, 0, vgpr_size * 4)
    else:
    ...
    ctypes.memset(self.vgpr_buf.gpu, 0, vgpr_size * 4)
    ctypes.memset(self.sgpr_buf.gpu, 0, SGPR_COUNT * 4)
...
def run_asm(lib: int, lib_sz: int, gx: int, gy: int, gz: int, lx: int, ly: int, lz: int, args_ptr: int, rsrc2: int = 0x19c,
  ...
  def _run_workgroup(gidx: int, gidy: int, gidz: int, tracing: bool):
    ...
    for wave_start in range(0, total_threads, wave_size):
      ...
      scratch_base = scratch_buf.gpu + (wave_start // wave_size) * scratch_size * wave_size if scratch_buf else 0
      waves.append((st, [ctypes.c_uint64(st.sgpr_buf.gpu), ctypes.c_uint64(st.vgpr_buf.gpu),
                         ctypes.c_uint64(vmem_buf.gpu), ctypes.c_uint64(lds_buf.gpu),
                         ctypes.c_uint64(scratch_base if scratch_buf else 0),
                         ctypes.c_uint64(st.accvgpr_buf.gpu)]))
  ...
  with _MXCSRContext():
    for gidz, gidy, gidx in itertools.product(range(gz), range(gy), range(gx)):
      ...
      if lds_size > 0: ctypes.memset(lds_buf.gpu, 0, max(lds_size, 4))  # reset LDS for next workgroup
