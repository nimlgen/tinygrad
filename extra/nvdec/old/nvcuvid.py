...
def show_frames():
  ...
  while not dec.frame_queue.empty():
    ...
    total = h + ch
    frab = dec.frame_queue.get()
    frame = np.frombuffer(frab, dtype=np.uint8).reshape((total, w))

    saver.append(frab)

    # print("lol")

    # if img_id in [0, 633, 1199]:
    #   im = Image.fromarray(frame)
    #   im.save("./extra/hevc/out/ref_frame_%04d.png" % img_id)
    # save image using cv2

    # os.makedirs("extra/hevc/out", exist_ok=True)

    # if img_id in [78, 633, 1022, 1199]:
    #   cv2.imwrite(f"extra/hevc/out/ref_frame_{img_id-1:04d}.png", cv2.cvtColor(frame, cv2.COLOR_YUV2BGR_NV12))
...
if __name__ == "__main__":
  ...
  # with Path("/home/nimlgen/comma_video.hevc").open("rb") as f:
  with Path("/home/nimlgen/Chunk_1/b0c9d2329ad1606b|2018-08-01--21-13-49/5/video.hevc").open("rb") as f:
    # for packet in iter(lambda: f.read(4096), b""):
    dec.decode(f.read())
    # show_frames()
  show_frames()
  # pickle.dump(saver, open("extra/hevc/out/cuda_decoded_frames.pkl", "wb"))
