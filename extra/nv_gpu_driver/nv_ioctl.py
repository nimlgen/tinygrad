...
def get_classes():
  ...
  known_classes = {"NV01_DEVICE_0", "NV01_ROOT", "NV1_MEMORY_SYSTEM", "NV01_MEMORY_VIRTUAL", "NV1_MEMORY_USER", "NV50_MEMORY_VIRTUAL", "NV_FERMI_VASPACE_A",
                   "NV20_SUBDEVICE_0", "NV01_ROOT_CLIENT", "NV01_ALLOC_MEMORY"}
...
def ioctl(fd, request, argp):
  ...
  if itype == ord(nv_gpu.NV_IOCTL_MAGIC):
    if nr == nv_gpu.NV_ESC_RM_CONTROL:
      ...
      if s.cmd in [0x20809009, 0x20809064, 0x20809001, 0x2080a028, 0x20808165]:
        print("skipping NV_ESC_RM_CONTROL cmd", hex(s.cmd))
        return 0
  ...
  if itype == ord(nv_gpu.NV_IOCTL_MAGIC):
    if nr == nv_gpu.NV_ESC_RM_CONTROL:
      ...
      if s.pAllocParms is not None:
        ...
        if s.hClass == nv_gpu.NV20_SUBDEVICE_0: dump_struct(get_struct(s.pAllocParms, nv_gpu.NV2080_ALLOC_PARAMETERS))
        ...
        if s.hClass == nv_gpu.NV01_ALLOC_MEMORY: dump_struct(get_struct(s.pAllocParms, nv_gpu.NVOS02_PARAMETERS))
        ...
        if s.hClass == nv_gpu.AMPERE_CHANNEL_GPFIFO_A:
          ...
          if sx.hObjectBuffer in gpus_virt_to_sys:
            gpus_fifo.append((sx.gpFifoOffset, sx.gpFifoEntries, next((x[0] for x in mappings_cache if x[1] == 196608), 0) + 0x20000, None)) # hack
      ...
      s = get_struct(argp, nv_gpu.NVOS33_PARAMETERS)
      print(f"NV_ESC_RM_MAP_MEMORY", end="")
      dump_struct_ext(s)
...
IDEN_MAPPINGS = False
def _dump_gpfifo(mark, iden=False):
  global IDEN_MAPPINGS
  IDEN_MAPPINGS = iden
  launches = []
  ...
  for start, size, start_sys, ud_sys in gpus_fifo:
    # print(old_gpputs[start], hex(start), hex(size), hex(start_sys), hex(ud_sys))
    if start_sys > 0 and not IDEN_MAPPINGS:
