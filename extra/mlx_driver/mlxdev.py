...
class MLXDev:
  ...
  def __init__(self, pci_dev:PCIDevice):
    ...
    assert (cmdif_sub >> 16) == self.CMD_IF_REV
    ...
    self.pd = self._alloc_24(mlx5.MLX5_CMD_OP_ALLOC_PD)
    self.td = self._alloc_24(mlx5.MLX5_CMD_OP_ALLOC_TRANSPORT_DOMAIN)
  ...
  def _setup_mbox(self, n, base, tok):
    ...
    for i in range(n):
      ...
      if i < n - 1:
        struct.pack_into('>Q', self.dma.mv, off + 560, self._dma_phys(0x1000 + (base + i + 1) * self.MBOX_STRIDE))
  ...
  def cmd_exec(self, opcode, op_mod=0, inp=b'', out_sz=0, page_queue=False):
    ...
    self.dma.mv[lay + 60] = tok
    self.dma.mv[lay + 63] = mlx5.CMD_OWNER_HW
  ...
  def _set_issi(self):
    ...
    if struct.unpack_from('>I', out, 100)[0] & (1 << 1):
      inp = bytearray(8)
      struct.pack_into('>I', inp, 0, 1)
  ...
  def _satisfy_pages(self, mode):
    ...
    npages = struct.unpack_from('>i', out, 4)[0]
  ...
  def _set_hca_caps(self):
    ...
    fill_ifc(cap, CAP, log_max_qp=min(18, read_ifc(gen_max, CAP, 'log_max_qp')))
    ...
    vhca_ports = read_ifc(gen_max, CAP, 'num_vhca_ports')
    if vhca_ports: fill_ifc(cap, CAP, num_vhca_ports=vhca_ports)
    ...
    max_msix = read_ifc(gen_max, CAP, 'num_total_dynamic_vf_msix')
    if max_msix: fill_ifc(cap, CAP, num_total_dynamic_vf_msix=max_msix)
    ...
    ATOM = mlx5.struct_mlx5_ifc_atomic_caps_bits
    if read_ifc(gen_cur, CAP, 'atomic'):
      self._query_cap(mlx5.MLX5_CAP_ATOMIC, 0)
      atom_cur = self._query_cap(mlx5.MLX5_CAP_ATOMIC, 1)
      if read_ifc(atom_cur, ATOM, 'supported_atomic_req_8B_endianness_mode_1'):
        cap = bytearray(4096)
        fill_ifc(cap, ATOM, atomic_req_8B_endianness_mode=1)
    ...
    if read_ifc(gen_cur, CAP, 'pg'):
      ...
      if ifc_get(odp_max, 0x600, 1) and ifc_get(odp_max, 0x245, 1):
        ifc_set(cap, 0x600, 1, 1); modified = True
      else:
    ...
    if not read_ifc(roce_cur, ROCE, 'sw_r_roce_src_udp_port') and read_ifc(roce_max, ROCE, 'sw_r_roce_src_udp_port'):
      ...
      fill_ifc(cap, ROCE, sw_r_roce_src_udp_port=1)
  ...
  def _alloc_uar(self):
    ...
    uar = struct.unpack_from('>I', out, 0)[0] & 0xFFFFFF
    page_sz = os.sysconf('SC_PAGE_SIZE')
    self.uar_page = self.pci_dev.map_bar(0, off=uar * page_sz, size=page_sz, fmt='I')
    return uar
  ...
  def _create_eq(self, log_eq_size=7):
    eq_size, n_pages = 1 << log_eq_size, ((1 << log_eq_size) * 64 + 0xFFF) // 0x1000
    ...
    eq_num = struct.unpack_from('>I', out, 0)[0] & 0xFF
    if MLX_DEBUG >= 1: print(f"mlx5: created EQ {eq_num}, {eq_size} entries")
    return eq_num

  def _create_cq(self, log_cq_size=7):
    cq_size, n_pages = 1 << log_cq_size, ((1 << log_cq_size) * 64 + 0xFFF) // 0x1000
    ...
    cqn = struct.unpack_from('>I', out, 0)[0] & 0xFFFFFF
  ...
  def _fill_qpc(self, qpc, **kwargs):
    fill_ifc(qpc, mlx5.struct_mlx5_ifc_qpc_bits, st=0, pm_state=3, pd=self.pd, cqn_snd=self.cq, cqn_rcv=self.cq, **kwargs)

  def _fill_ads(self, qpc, **kwargs):
    fill_ifc(qpc, mlx5.struct_mlx5_ifc_ads_bits, base=0xC0, **kwargs)

  def _create_qp(self, log_sq_size=4, log_rq_size=4):
    log_rq_stride = 0
    rq_sz = (1 << log_rq_size) << (log_rq_stride + 4)
    n_pages = ((rq_sz + (1 << log_sq_size) * 64) + 0xFFF) // 0x1000

    self.qp_buf, qp_paddrs = self.pci_dev.alloc_sysmem(n_pages * 0x1000)
    self.qp_dbr_phys = self._alloc_dbr()
    self.sq_offset = rq_sz

    inp = bytearray(264 + n_pages * 8)
    self._fill_qpc(memoryview(inp)[16:248], log_msg_max=30, log_rq_size=log_rq_size, log_rq_stride=log_rq_stride,
                   log_sq_size=log_sq_size, rlky=1, uar_page=self.uar, log_page_size=0, dbr_addr=self.qp_dbr_phys)
    pack_pas(inp, 264, qp_paddrs)

    out = self.cmd_exec(mlx5.MLX5_CMD_OP_CREATE_QP, inp=inp)
    qpn = struct.unpack_from('>I', out, 0)[0] & 0xFFFFFF
    if MLX_DEBUG >= 1: print(f"mlx5: created QP 0x{qpn:x} (RC, sq={1<<log_sq_size} rq={1<<log_rq_size})")

    inp2 = bytearray(264)
    struct.pack_into('>I', inp2, 0, qpn)
    self._fill_qpc(memoryview(inp2)[16:248], log_ack_req_freq=8)
    self._fill_ads(memoryview(inp2)[16:248], pkey_index=0, vhca_port_num=1)
  ...
  def set_roce_address(self, gid_index, ip):
    inp = bytearray(40)
    struct.pack_into('>H', inp, 0, gid_index)
    inp[3] = 1
    ra = memoryview(inp)[8:]
    ra[0:16] = self._ipv4_to_gid(ip)
    ra[18:20] = self.mac[0:2]
    ra[20:24] = self.mac[2:6]
  ...
  def init2rtr(self, remote_qpn, remote_mac, remote_gid):
    ...
    struct.pack_into('>I', inp, 8, 0x1A)  # opt_param_mask: RRE|RWE|PKEY_INDEX (no RAE — avoids atomic_mode conflict)

    qpc = memoryview(inp)[16:248]
    CAP = mlx5.struct_mlx5_ifc_cmd_hca_cap_bits
    self._fill_qpc(qpc, mtu=3, log_msg_max=read_ifc(self.gen_caps, CAP, 'log_max_msg'), remote_qpn=remote_qpn,
                   log_ack_req_freq=8, log_rra_max=3, rre=1, rwe=1, min_rnr_nak=12, next_rcv_psn=0)
    self._fill_ads(qpc, pkey_index=0, src_addr_index=0, hop_limit=64,
  ...
  def rtr2rts(self):
    ...
    struct.pack_into('>I', inp, 0, self.qpn)
    qpc = memoryview(inp)[16:248]
    self._fill_qpc(qpc, log_ack_req_freq=8, next_send_psn=0, log_sra_max=3, retry_count=7, rnr_retry=7)
    self._fill_ads(qpc, ack_timeout=14, vhca_port_num=1)
  ...
  def _post_sq(self, opcode, ds_count, segs):
    wqe_off = self.sq_offset + (self.sq_head & 0xF) * 64
    wqe = memoryview(self.qp_buf.mv)[wqe_off:wqe_off + 64]
    wqe[:64] = bytes(64)
    struct.pack_into('>I', wqe, 0, (self.sq_head << 8) | opcode)
    struct.pack_into('>I', wqe, 4, (self.qpn << 8) | ds_count)
    wqe[11] = 0x08
    for off, data in segs: wqe[off:off + len(data)] = data
    self.sq_head += 1
    dbr_off = self.qp_dbr_phys - self.dbr_paddrs[0]
    struct.pack_into('>I', self.dbr_mem.mv, dbr_off + 4, self.sq_head)
    bf_off = 0x800 // 4
  ...
  def send(self, addr, lkey, length):
    self._post_sq(0x0a, 2, [(16, struct.pack('>IIQ', length, lkey, addr))])
    if MLX_DEBUG >= 1: print(f"mlx5: SEND {length}B complete")

  def post_recv(self, addr, lkey, length):
    wqe_off = (self.rq_head & 0xF) * 16
    struct.pack_into('>IIQ', self.qp_buf.mv, wqe_off, length, lkey, addr)
    self.rq_head += 1
    dbr_off = self.qp_dbr_phys - self.dbr_paddrs[0]
    struct.pack_into('>I', self.dbr_mem.mv, dbr_off, self.rq_head & 0xFFFF)
